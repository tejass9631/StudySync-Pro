...
onAuthStateChanged(auth, async (user) => {
  ...
  // ✅ SHOW UI ONLY AFTER AUTH CONFIRM
  document.body.style.display = "block";
  ...
  if (todayEl) {
    if (data.today) {
      todayEl.innerText = `${data.today.completed}/${data.today.total}`;
    } else {
      todayEl.innerText = "0/0";
    }
  }
...
// check karo ki 4th item exist karta hai ya nahi
